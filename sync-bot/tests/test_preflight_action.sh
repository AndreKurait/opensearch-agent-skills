...
#   push    AndreKurait (unprotected)              0 (OK)
#   push    opensearch-project (protected)         1 (fail)
#   pr      AndreKurait (unprotected)              0 (OK)
...
run_case "pr -> unprotected fork main (expect OK)" \
  pr main AndreKurait/opensearch-agent-skills true 0 || FAIL=1
...
run_case "bogus mode (expect FAIL on validation)" \
  pushh main AndreKurait/opensearch-agent-skills true 1 || FAIL=1
