...
2. **Mirror it from an upstream repo** — register a YAML source under `sync/sources/` and the sync framework replays the upstream history into `skills/` every 5 minutes. See *Mirroring an Upstream Skill* further down.
...
- A scheduled GitHub Action (`Sync Skills`) runs every 5 minutes.
